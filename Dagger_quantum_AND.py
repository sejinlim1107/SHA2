...
def quantum_and_dag(eng, a, b, c):
    ...
    #with Dagger(eng):  # Dagger는 역함수 개념. Uncompute는 명령어 역순 수행
...
def adder_test(eng, A, B, n):
    ...
    sum, aa, bb, anan = outDraper_adder(eng,a,b,n)

    if (resource_check == 0):
        print('Add result : ', end='')
        print_vector(eng, sum, n+1)
        print('aa result : ', end='')
        print_vector(eng, aa, n)
        print('bb result : ', end='')
        print_vector(eng, bb, n)
        print('anan result : ', end='')
        length = n - w(n) - floor(log2(n))
        print_vector(eng, anan, length)
