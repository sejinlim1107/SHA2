...
class WallaceTreeAdder(BaseTreeAdder):
        ...
        while (t):
            ...
            if (len(w[n - 1]) > 2):
                while (len(w[n - 1]) > 2):
                    ...
                    stack.append([c, b, a, 0, 0, 0])
                    circuit.append(self.modQFA(c, b, a))
                    # self.countQFA +=1
                    w_tmp.append(a)
                    #w_tmp2.append(carry)
            ...
            for i in reversed(range(0, n - 1)):
                ...
                while (len(w[i]) > 2):
                    ...
                    an.append(carry)
                    num += 1
                    stack.append([c, b, a, carry])
                    circuit.append(self.QFA(c, b, a, carry))
                    # self.countQFA +=1
                    w_tmp.append(a)
                    w_tmp2.append(carry)
        ...
        while (t):
            ...
            if (len(w[n - 1]) > 2):
                ...
                if len(w[i]) == 2: # 최상위 비트
                    ...
                    stack.append([b, a, 0, 0, 0])
                    circuit.append(cirq.CNOT(b, a)) # modQHA
                    # self.countQHA += 1
                    w_tmp.append(a)
                    #w_tmp2.append(carry)
            ...
            for i in reversed(range(0, n - 1)):
                ...
                if len(w[i]) == 2:
                    ...
                    an.append(carry)
                    num += 1
                    stack.append([b, a, carry])
                    circuit.append(self.QHA(b, a, carry))
                    # self.countQHA +=1
                    w_tmp.append(a)
                    w_tmp2.append(carry)
    ...
    def apply_final_cnot(self, circuit, w, num):
        ...
        an.append(cirq.NamedQubit("csa_carry" + str(num)))
    ...
    def reverse_adders(self, circuit, stack, rca_A, list1):
        print("stack",stack)
        print(len(stack))
        while (len(stack)):
            tmp = stack.pop()
            tmp2 = [tmp[-2], tmp[-1]]
            if len(tmp) == 5 or len(tmp) == 6:
                tmp2 = [tmp[-5], tmp[-4]]
                print(tmp2)
            if self.adder_type == 'InplaceAdder':
                if (len(list(set(list1) & set(tmp2))) == 0):
                    ...
                    if (len(list(set(list1) & set([tmp2[1]]))) == 1): # carry
                        print("second")
                        if (len(tmp) == 3):
                            circuit.append(cirq.CNOT(tmp[0], tmp[1]))
                        if (len(tmp) == 4):
    ...
    def construct_circuit(self):
        ...
        print("rca_A", rca_A)
        print("rca_B", rca_B)
        circuit, result = self.apply_adder(circuit, rca_B, rca_A) # A에 저장
        circuit = self.reverse_adders(circuit, stack, rca_A, rca_A + R)
...
if __name__ == '__main__':
    ...
    circuit.append(cirq.measure(C, key="C"))
    circuit.append(cirq.measure(an, key="qb"))
    print(k.result)
    # circuit.append(cirq.measure(k.result, key="result"))
